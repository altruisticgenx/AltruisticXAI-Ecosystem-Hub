            ...
            {pillars.map((pillar) => {
              ...
              return (
                <motion.div key={pillar.id} variants={itemVariants} transition={itemTransition}>
                  <Link to={pillar.href}>
                    <Card className="<div\n  class=\"group/card relative isolate grid w-full max-w-[18rem] min-h-[9rem]\n  grid-rows-[auto,1fr,auto] rounded-2xl border border-border/60 bg-card/90\n  p-3 sm:p-4 text-card-foreground shadow-[0_1px_2px_rgba(0,0,0,.08),0_8px_24px_-8px_rgba(0,0,0,.12)]\n  backdrop-blur supports-[backdrop-filter]:bg-card/80\n  transition-all duration-300 ease-out will-change-transform transform-gpu\n  motion-safe:hover:-translate-y-1 motion-safe:hover:rotate-[0.6deg]\n  motion-safe:hover:shadow-[0_14px_40px_-14px_rgba(0,0,0,.2)]\n  focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60\n  before:absolute before:inset-0 before:-z-10 before:rounded-[inherit]\n  before:opacity-0 before:transition-opacity before:duration-300\n  before:bg-[radial-gradient(120px_60px_at_var(--mx,50%)_var(--my,40%),theme(colors.primary/25),transparent_60%)]\n  hover:before:opacity-100\"\n  tabindex=\"0\"\n>\n  <!-- top row: badge / icon -->\n  <div class=\"flex items-center gap-2\">\n    <span class=\"inline-flex items-center rounded-full border border-border/60 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide bg-background/60\">Live</span>\n    <span class=\"text-[11px] opacity-70\">~2 min setup</span>\n  </div>\n\n  <!-- middle: content -->\n  <div class=\"mt-2\">\n    <h3 class=\"text-sm sm:text-base font-semibold leading-tight\">Smart Micro-Pilot</h3>\n    <p class=\"mt-1 text-xs sm:text-sm leading-5 opacity-80\">\n      Launch a bite-size pilot with built-in analytics, ROI snapshots, and exportable briefs.\n    </p>\n  </div>\n\n  <!-- bottom: actions -->\n  <div class=\"mt-3 flex items-center justify-between\">\n    <button class=\"inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition\">\n      Try it\n      <svg class=\"size-3.5\" viewBox=\"0 0 24 24\" fill=\"none\"><path d=\"M7 17l8-5-8-5v10z\" stroke=\"currentColor\" stroke-width=\"1.5\"/></svg>\n    </button>\n    <a class=\"text-xs underline underline-offset-4 opacity-80 hover:opacity-100\" href=\"#\">Details</a>\n  </div>\n</div>\n">
